...
init();
    ...
    if (scheduler.getState().mode == "week") {
        ...
        var eventURL = "/student/event/existed/" + roomID + 
                       "/" + min_date.getFullYear() +
                       "/" + (min_date.getMonth()+1) + "/" + min_date.getDate() +
                       "/" + max_date.getFullYear() + "/" + (max_date.getMonth()+1) + 
                       "/" + max_date.getDate() ;
        ...
        $.ajax({
          ...
          success: function(result){
              if(result.id == 0 ){
                  ...
                  document.getElementById('successMessage').innerHTML = "You have successfully created a reservation!";
              }     
            },
    ...
    scheduler.attachEvent("onEventChanged", function(event_id,event_object){
      ...
      $.ajax({
          async: false,
          type: "POST",
          url: "/student/event/update/" + event_id ,
          data: {
        	  year: year,
              month: month,
              day: day,
          ...
          success: function(result){
        	  if(result.id == 0 ){
                  ...
                  $("#errorMessage").css("display", "none");
                  $("#successMessage").css("display", "block");
                  document.getElementById('successMessage').innerHTML = "You have successfully changed a reservation!";
                  isSuccess = true;
              }        
          },
          dataType:  "json"
    ...
    scheduler.attachEvent("onConfirmedBeforeEventDelete", function(event_id){
        ...
        if(event_id > 0){
    	  $.ajax({
              async: false,
              type: "POST",
              url: "/student/event/delete/" + event_id ,
              success: function(result){
            	  if(result.id == 0 ){ 
            		  $("#successMessage").css("display", "none");
                      $("#errorMessage").css("display", "block");
	            	  ...
	            	  $("#successMessage").css("display", "block");
	            	  document.getElementById('successMessage').innerHTML = "You have successfully deleted a reservation!";
	            	  isSuccess = true;
            	  }
              },
           });
